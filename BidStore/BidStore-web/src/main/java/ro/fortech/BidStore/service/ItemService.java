...
public class ItemService {
	...
	public List<Item> createItems(int i, boolean toSell) {
		
		List<Item> items = new ArrayList<Item>();
		
		for (i=1;i<=100;i++) {
			Item item = new Item(i, "Item"+i, "Photo"+(100-1-i), bigDescription, i*10, new Date(System.currentTimeMillis()+1000*60*60*24*10), i);
			if (item.isToSell() == toSell) items.add(item);
		}
